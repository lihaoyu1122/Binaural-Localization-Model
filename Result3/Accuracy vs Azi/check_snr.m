...
[back_noise,~] = audioread("backnoise_snr-15.wav");
for rand_i = 1:1:100
    audio = 10*transpose(0.01 * randn(1,10*sr));
    Fs = 48000;
for theta = ([90 270]) / 180*pi
...
y_sound_l = y_sound_l + back_noise(:,1);
y_sound_r = y_sound_r + back_noise(:,2);
%% find possibility
...
surf(X,Y,Z,C);
title("Direction possibility, surface represents possibility on downleft point");
xlabel('x');
ylabel('y');
zlabel('z');
clbar = colorbar;
...
fprintf(fileID,'%d %d ',max_theta_i*5,max_phi_i*5);
fclose(fileID);
end
fileID = fopen('output.txt','a');
fprintf(fileID,'\n');
