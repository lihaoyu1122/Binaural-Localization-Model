...
add_ILD = 0;%dB
if(add_ITD>0)
    n_point = round(add_ITD*sr/1000000);
    y_sound_l = [zeros(n_point,1);y_sound_l];
    y_sound_r = [y_sound_r;zeros(n_point,1)];
    add_ITD = n_point * 1000000 / sr;
end
if(add_ITD<0)
    n_point = round(-add_ITD*sr/1000000);
    y_sound_l = [y_sound_l;zeros(n_point,1)];
    y_sound_r = [zeros(n_point,1);y_sound_r];
    add_ITD = n_point * 1000000 / sr;
end
if(add_ILD>0)
    y_sound_r = y_sound_r * (10^(add_ILD/10));
end
if(add_ILD<0)
    y_sound_l = y_sound_l * (10^(-add_ILD/10));
end
%% find possibility
